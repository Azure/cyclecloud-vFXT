...
class Azure_test(tests.vFXTTestCase.Base):
    ...
    def test_move_route(self):
        ...
        try:
            ...
            free_addrs, subnet = service.get_available_addresses(addr_range='172.16.0.0/16') #pylint: disable=unused-variable
            i1.add_address(free_addrs[0])
            i2.add_address(free_addrs[0])
            i1.refresh()
            i2.refresh()
            self.assertTrue(free_addrs[0] not in i1.in_use_addresses())
            self.assertTrue(free_addrs[0] in i2.in_use_addresses())
    ...
    def test_cache_to_disk_config(self):
        ...
        self.assertTrue(service._cache_to_disk_config(500) == (1, 512))
        self.assertTrue(service._cache_to_disk_config(512) == (1, 512))
        self.assertTrue(service._cache_to_disk_config(750) == (2, 512))
        self.assertTrue(service._cache_to_disk_config(1000) == (2, 512))
