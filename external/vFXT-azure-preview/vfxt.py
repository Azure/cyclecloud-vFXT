...
def main():
    ...
    azure_opts = parser.add_argument_group('Azure specific options', 'Options applicable for --cloude-type azure')
    ...
    if args.cloud_type == 'aws':
        ...
        if args.ssh_key:
            try:
                with open(args.ssh_key) as f:
                    ssh_key_data = f.read()
                    assert 'rsa' in ssh_key_data, "the SSH key must be of type RSA"
            except Exception as e:
                logger.error("Failed to read SSH key: {}".format(e))
                parser.exit(1)
