...
class Service(ServiceBase):
    ...
    def get_instance_data(cls, **options):
        ...
        try:
            ...
            while True:
                try:
                    url_path = '/metadata/identity/oauth2/token?api-version=2018-02-01&resource=https://management.azure.com/'
                    conn.request('GET', '{}'.format(url_path), headers=headers)
                    response = conn.getresponse()
                    if response.status == 200:
    ...
    def stop(self, instance, wait=WAIT_FOR_STOP):
        ...
        op = conn.virtual_machines.deallocate(self.resource_group, self.name(instance))
    ...
    def start(self, instance, wait=WAIT_FOR_START):
        ...
        conn = self.connection()
        op = conn.virtual_machines.start(self.resource_group, self.name(instance))
    ...
    def restart(self, instance, wait=ServiceBase.WAIT_FOR_RESTART):
        ...
        conn = self.connection()
        op = conn.virtual_machines.restart(self.resource_group, self.name(instance))
    ...
    def destroy(self, instance, wait=WAIT_FOR_DESTROY):
        ...
        try:
            if instance.storage_profile.os_disk.managed_disk:
                conn.disks.delete(self.resource_group, instance.storage_profile.os_disk.name)
        ...
        for data_disk in instance.storage_profile.data_disks:
            try:
                if data_disk.managed_disk:
                    conn.disks.delete(self.resource_group, data_disk.name)
                elif data_disk.vhd:
        ...
        for nic_ref in instance.network_profile.network_interfaces:
            ...
            try:
                ...
                for public_addr in [config.public_ip_address.id.split('/')[-1] for config in nic.ip_configurations if config.public_ip_address]:
                    op = netconn.public_ip_addresses.delete(nic_rsg, public_addr)
                    # skip self._wait_for_operation(op, msg='route to be deleted')
            except Exception as e:
                log.debug("Failed to delete NIC: {}".format(e))

        for rt in netconn.route_tables.list(self.network_resource_group):
            for route in rt.routes:
                ...
                try:
                    ...
                    op = netconn.routes.delete(self.network_resource_group, route_table_name=table_name, route_name=route_name)
                    # skip self._wait_for_operation(op, msg='route to be deleted')
                except Exception as e:
    ...
    def _instance_network(self, instance):
        ...
        primary_nic = self._instance_primary_nic(instance)
        subnet_parts = primary_nic.ip_configurations[0].subnet.id.split('/')
        return self._get_network(resource_group=subnet_parts[4], network=subnet_parts[8])
    ...
    def status(self, instance):
        ...
        '''
        conn = self.connection()
        return [_.code for _ in conn.virtual_machines.instance_view(self.resource_group, instance.name).statuses]
    ...
    def create_instance(self, machine_type, name, boot_disk_image, other_disks=None, **options):
        ...
        if availability_set:
            try:
                a_set = conn.availability_sets.get(self.resource_group, availability_set)
        ...
        # if its a marketplace path like OpenLogic:CentOS:7.1:latest
            ...
            try:
                img = conn.images.get(self.resource_group, boot_disk_image)
                body['storage_profile']['image_reference'] = {'id': img.id}
            except Exception as e:
        ...
        network_resource_group = options.get('network_resource_group') or self.network_resource_group
        public_ip_address = options.get('auto_public_address', False)
        nic = None
        try:
            nic = self._create_nic('{}-1-NIC'.format(name),
                network=network,
                subnet=subnet,
        ...
        try:
            ...
            instance = conn.virtual_machines.get(self.resource_group, name)
            ...
            try: # it seems we have to manually delete a failed instance
                op = conn.virtual_machines.delete(self.resource_group, name)
            ...
            if public_ip_address:
                try:
                    op = self.connection('network').public_ip_addresses.delete(self.network_resource_group, '{}-public-address'.format(name))
            ...
            # delete nic
            try:
                self.connection('network').network_interfaces.delete(self.network_resource_group, '{}-1-NIC'.format(name))
            ...
            if body['storage_profile']['os_disk']['create_option'] != 'Attach':
                try:
                    conn.disks.delete(self.resource_group, root_disk_name)
                ...
                for disk in other_disks:
                    try:
                        if disk.get('managed_disk') and disk.get('name'):
                            conn.disks.delete(self.resource_group, disk['name'])
                        else:
    ...
    def create_node(self, node_name, cfg, node_opts, instance_options):
        ...
        node_num = int(node_name[-2:])
        # stagger the autoscale commands
        time.sleep(2*node_num)
        user_data = cfg
        ...
        filter_expr = 'ClusterName=="%s" && Name =="vfxt" && IsArray == true' % cluster_name
        res = ds.find("Cloud.node", filter_expr=filter_expr)

        try:
            target_count = int(res[0]['TargetCount'])
        except:
            target_count = 0

        autoscale_array = {
                'Name': 'vfxt',
                'TargetCount': node_num,
                'UserData' : cfg
                }

        autoscale_array_str = '[{ "Name": "vfxt", "TargetCount": %s, "UserData" : %s}]' % (node_num,cfg)

        if (node_num > target_count):
            log.info("updating userdata = %s" % cfg)
            log.info("%d > %d, autostarting node %d " % (node_num,target_count,node_num))
        ...
        sleep_time = 10
        while True:
            filter_expr = 'ClusterName=="%s" && Template=="vfxt"' % cluster_name
            res = ds.find("Cloud.node", filter_expr=filter_expr)
            _instances = {}
            _states = {}
            _rg = {}
            ...
            for node in res:
                if count >= count_max:
                    raise ValueError("node %d not becoming ready" % node_num)
                try:
    ...
    def old_create_node(self, node_name, cfg, node_opts, instance_options):
    ...
    def add_cluster_nodes(self, cluster, count, **options):
        ...
        '''
        subnets         = options.get('subnets') or []
        subnets         = [subnets] if isinstance(subnets, basestring) else subnets
    ...
    def load_cluster_information(self, cluster, **options):
        ...
        if instances:
            ...
            try:
                nic_id = instances[0].network_profile.network_interfaces[0].id
                nic_name = nic_id.split('/')[-1]
                nic_rsg = nic_id.split('/')[4]
                interface = self.connection('network').network_interfaces.get(nic_rsg, nic_name)
                if interface.network_security_group:
                    cluster.service.network_security_group = interface.network_security_group.id.split('/')[-1]
            except Exception as e:
                if not log.isEnabledFor(logging.DEBUG):
                    log.exception(e)
    ...
    def _commit_instance(self, instance, body):
        ...
        conn = self.connection()
        try:
            op = conn.virtual_machines.create_or_update(self.resource_group, instance.name, body)
    ...
    def _create_role(self, name, **options):
        ...
        # XXX will this work? The role exists in the vm resource group but allows both groups?
        if self.network_resource_group != self.resource_group:
    ...
    def _get_network(self, resource_group=None, network=None):
...
def _get_nicid_from_cyclecloud():
    ...
    res = ds.find("Cloud.node", filter_expr=filter_expr)
    return res[0]['azureresources']['networkinterfaceids'][0]